...
def handle_list_queries(query):
    ...
    matched_keywords = []
    for category, keywords in list_keywords.items():
        if any(kw in query_lower for kw in keywords):
            matched_keywords.append(category)

    # Filter DataFrame based on matched keywords in summary or description
    if matched_keywords:
        filtered_rows = df[df.apply(lambda row: any(kw in row['summary'].lower() or kw in row['description'].lower() for kw in matched_keywords), axis=1)]
        if not filtered_rows.empty:
            return filtered_rows.to_dict(orient='records')
        else:
            return "No matching issues found."
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print("4. Exit")
        user_choice = input("Your choice (1/2/3/4): ")
        ...
        if user_choice == "1":
            ...
            print("Bot: Invalid choice. Please choose between 1, 2, 3, or 4.")
